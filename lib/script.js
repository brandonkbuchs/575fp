...
var globalMap, globalOutput, lat, lng;
...
function handleHover(data){
	document.querySelectorAll("svg path").forEach((path, index) => {
    	let row = data[index],
      	...
      	path.addEventListener("mouseenter", handleMouseenter(lat, lng));
      	path.addEventListener("mouseleave", handleMouseleave);
    });
};

//function for mouse entering particular path
function handleMouseenter(e, lat, lng){
    ...
    var region = 
        fetch(`https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/reverseGeocode?f=pjson&langCode=EN&location=${lng},${lat}`)
            .then(res => res.json())
            .then(myJson => { (myJson.address.City + ', ' + myJson.address.Region)
            }); 
    ...
    console.log(region);
  	...
  	globalOutput.textContent = `${region}, Census Block: ${geoid10}, Location Index: ${locIndex}, Walk Index: ${walkIndex}`;
}
